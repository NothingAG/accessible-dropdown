...
function onKeyup(event) {
    let currentlySelectable = elems.arrowSelectableElems.filter(elem => !elem.hidden)
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        ...
        for (let i = 0; i < elems.arrowSelectableElems.length; i++) {
            ...
            if(!currentElem.hidden) {
                lastSelected = j;
                let elemToFocus = currentElem === elems.input ? elems.input : currentElem.querySelector('input');
                elemToFocus.focus();
                break;
            }
        }
